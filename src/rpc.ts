...
import { resolveProject, startNextTask, startTask } from './session-link.ts'
...
async function dispatch(
  ...
  switch (method) {
    ...
    case 'task.get': {
      ...
      return { task, comments: board.listComments(id), activity: board.listActivity(id) }
    }
    case 'task.create':
    ...
    case 'board.view': {
      ...
      return {
        ...
        scheduler: ctx.reflect.get('taskboardScheduler')?.state(project.id)
          ?? { concurrency: 1, autoPull: true, running: 0, waiting: 0 },
      }
    }
    case 'task.accept': {
      ...
      const accepted = await board.updateTask(id, { status: 'done' }, {
        actor: LOCAL_USER,
        ...(expectedVersion !== undefined ? { expectedVersion } : {}),
      })
    ...
    case 'task.sendBack': {
      ...
      const sentBack = await board.updateTask(id, { status: 'todo', sessionId: undefined }, {
        actor: LOCAL_USER,
        ...(expectedVersion !== undefined ? { expectedVersion } : {}),
      })
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (typeof method !== 'string' || !(TASKBOARD_METHODS as readonly string[]).includes(method)) {
          sendJson(res, 400, { ok: false, code: 'unknown-method', message: `unknown method "${String(method)}"` })
          return
        }
